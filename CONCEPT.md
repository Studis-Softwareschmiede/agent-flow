...
**Notiz-Front-Door (Obsidian-Ingest, Konzept akzeptiert — noch nicht gebaut):** Neben der getippten vagen Anforderung kann ein **Obsidian-Projektordner** (mehrere freie `.md`-Notizen) als Requirement-Quelle dienen. Ein Fabrik-Befehl (Arbeitstitel `/agent-flow:from-notes <ordner>`) verarbeitet die Notizen in **drei Stufen** — (a) Notiz → `docs/concept.md`, (b) Konzept → `docs/specs/<feature>.md` (+ `architekt`/`dba` wo nötig), (c) Spec → Board-Items **über den bestehenden `requirement`-Agenten** — wobei jede Stufe bei klaren Notizen automatisch durchläuft und bei Unklarheiten mit **genau einem gesammelten Fragenkatalog** anhält (Stufe a mit niedrigster Nachfrage-Schwelle, damit sich Fehler nicht in Spec/Stories fortpflanzen); jede Stufe wird einzeln committet. Der verknüpfte Ordner bleibt am Projekt vermerkt (`profile.obsidian_source`); ein **Re-Sync-Modus** (`--sync`) zeigt später Widersprüche zwischen Notiz- und Konzept/Spec-Stand auf — anders als `reconcile` (Code gewinnt automatisch) wird hier **nichts** automatisch überschrieben (Notizen sind unfertige Gedanken → je Divergenz ein Mensch-Entscheid). **Idea-Roundtrip (07.07.2026):** die Kette ist durchgängig ID-verankert — Ideennotizen tragen `IDEA-NNN` (Frontmatter), Konzeptabschnitte `C-NNN (← IDEA-NNN)`, Specs referenzieren `(← C-NNN)`; die Fabrik darf im Vault ausschliesslich **zwei definierte Zonen** schreiben (Frontmatter-Sync-Felder + generierter Abschnitt `## Stand aus Konzept (generiert)`), die persönliche Ausarbeitung ist tabu. Ein **`--audit`-Modus** prüft read-only die gesamte Kette IDEA→C→Spec→BR→Story→`@trace` auf Waisen und Widersprüche (abgeleitete Coverage-Map, nie handgepflegt); der Rückkanal Repo→Vault ist `reconcile` **Stufe 3**. Verträge: `docs/architecture/obsidian-ingest-subsystem.md`, `docs/specs/obsidian-ingest.md`, `docs/specs/obsidian-sync.md`. **Cross-Repo:** der Button + die Anzeige/Beantwortung des Fragenkatalogs leben in dev-gui (separate Story).
...
**Reconcile (rückwärtige Aufholung, Konzept akzeptiert — noch nicht gebaut):** Ein Button im „Spezifikation"-Reiter (dev-gui) startet den Fabrik-Befehl `/agent-flow:reconcile`, der die Doku eines Projekts wieder in Deckung bringt — **Stufe 1 (Form):** Specs ohne aktuellen `spec_format: use-case-2.0` werden in die neueste Vorlage konvertiert (läuft immer); **Stufe 2 (Inhalt, nur bei leerem Kanban):** Code ist maßgebend → Doku wird automatisch nachgezogen; **Stufe 3 (Obsidian-Rückspielung, nur wenn `profile.obsidian_source` gesetzt; neu 07.07.2026):** Änderungen mit konzeptioneller Tragweite (`C-NNN`-Abschnitte) fliessen in die generierte Zone der verankerten Ideennotiz zurück, Waisen ohne Ideen-Herkunft erzeugen eine neue, klar gekennzeichnete Ideennotiz; Drei-Wege-Abgleich via `last_sync`/`sync_hash`, beidseitige Änderung → Mensch-Entscheid, gelöscht wird nie (`superseded`). Alle Stufen legen genau **einen Diff** zur Freigabe vor (Vault-Patches werden im PR protokolliert und erst nach Merge ausgeführt); ein knappes `docs/spec-audit.md` protokolliert die Änderungen. Vertrag: `docs/architecture/reconcile-subsystem.md`.
...
- `.claude/memory.md` — **kuratierter, kurzer Projekt-Stand** (Aktueller Stand / Letzte Arbeiten / Offene Fäden, Deckel 60 Zeilen); Single-Writer `/flow` (Kurations-Schritt am Session-Ende), Orientierung — nie Wahrheit (bei Widerspruch gelten Board/Specs). Spec: [`docs/specs/project-memory.md`](docs/specs/project-memory.md).
...
**Entschieden (Spec-Status-Lebenszyklus):** Der Spec-Status (Frontmatter `status:`) kennt **genau drei** Werte: **`draft → active → superseded`** (`draft` = in Arbeit, gilt noch nicht; `active` = verbindlich/in Kraft; `superseded` = durch eine neuere Spec ersetzt). **`approved` ist KEIN Status** — es ist historisch von Hand in einzelne Specs gerutscht (kein Agent/Skill/Template erzeugt es; der einzige Konsument `board-cli` R2 wertet ausschliesslich `active`), faktisch ein Synonym für `active`. **Eine Wahrheit über alle Stellen:** Vorlage (`templates/_docs/specs/_template.md`), `docs/architecture/board-subsystem.md`, die `board-cli`-Regel und die dev-gui-Status-Filter führen dieselben drei Werte. Bestehende `approved`-Specs werden bei der Umsetzung auf `active` normalisiert (reiner Doc-Nachzug, kein Verhaltenscode) — die Umstellung selbst ist noch nicht erfolgt.

**Entschieden (Idea-Roundtrip, 07.07.2026) `[C-002 ← IDEA-002]`:** Der Kreislauf **Idee → Konzept → Spec → Realisierung → zurück zur Idee** ist geschlossen und ID-verankert. (1) **ID-Kette** `IDEA-NNN → C-NNN → Spec → BR → Story → @trace` — `from-notes` Stufe a vergibt/stempelt die IDs. (2) **Vault-Schreibzonen statt Read-only:** die Fabrik darf in Ideennotizen genau zwei Zonen anfassen (Frontmatter-Sync-Felder `idea_id`/`idea_status`/`last_sync`/`sync_hash` + generierter Abschnitt `## Stand aus Konzept (generiert)`); persönliche Ausarbeitung tabu, gelöscht wird nie (`superseded`). (3) **Rückkanal = `reconcile` Stufe 3** (nur bei gesetztem `profile.obsidian_source`): Konzept-Tragweite fliesst in die generierte Zone zurück, Code-first-Waisen erzeugen eine neue, gekennzeichnete Ideennotiz; Drei-Wege-Abgleich via `last_sync`/`sync_hash`, beidseitige Änderung → Mensch-Entscheid; Vault-Patches werden im Reconcile-PR protokolliert und erst nach Merge ausgeführt. (4) **`from-notes --audit`**: read-only Integritätsprüfung der ganzen Kette (Waisen abwärts/aufwärts, Widersprüche; `parked`/`rejected` = Entscheidung, keine Lücke). Verträge: `docs/architecture/obsidian-ingest-subsystem.md` (§4b/§5a) + `reconcile-subsystem.md` (§3 Stufe 3).

**Entschieden (Selbst-Dokumentations-Pflicht, 07.07.2026) `[C-001 ← IDEA-001]`:** Befund des Obsidian↔Repo-Abgleichs vom 30.06.2026: `CONCEPT.md`/`AGENTS.md` wachsen dem Repo hinterher, weil kein aktiver Agent sie nachpflegt — `train`/`retro` schreiben nur Packs/Agent-Defs, der einzige Agent mit Doku-Nachzug-Schritt (`teamLeader`) ist inaktiv („SPÄTER, nicht P1"). Konkret undokumentiert: der `estimator`-Agent (fehlt im AGENTS.md-Roster) und die gewachsenen `knowledge/`-Unterordner (`_meta/`, `build/`, `frameworks/`, `migration/`, `quality/` — fehlen in §4c). Entscheid (Umfang bewusst reduziert): (1) **`train`-Pflichtschritt** — der Bootstrap eines Packs in einer **neuen Kategorie** (neuer Unterordner) zieht `CONCEPT.md` §4c im **selben PR** nach; (2) **Roster-Nachzug** — `estimator` (und weitere ad-hoc entstandene Agenten) werden in `AGENTS.md` nachdokumentiert; (3) **kein** neuer periodischer Drift-Scan — die rückwärtige Aufholung ist durch `/agent-flow:reconcile` (Stufe 1/2, bei Obsidian-Anbindung Stufe 3) abgedeckt; `teamLeader` bleibt unverändert „SPÄTER". Herkunft: Ideennotiz `Agent Flow – Architektur.md` (§Gap zum Konzept, IDEA-001).
