...
- **ID-Brücke (Pflicht).** Bisher = GitHub-Issue-Nummer (int) als `item`. `/flow` schreibt den **numerischen Anteil der Story-ID** als `item`-Schlüssel (int) in `dispatches.jsonl`/`items.jsonl` — `S-014` → `14` (metrics-subsystem §2.1/§2.2, board-subsystem §4.4). Die Story-YAML spiegelt `dispo_*`/`tok` per ID-Join; die Ledger bleiben SoT der Ist-Werte.
...
`/flow` schreibt den **numerischen Anteil der Story-ID als int** (`S-014` → `14`) als `item`-Feld in jede `dispatches.jsonl`- und `items.jsonl`-Zeile (statt der GitHub-Issue-Nummer). Alle Metrik-Felder bleiben unverändert; nur der Schlüssel wechselt (board-subsystem §4.4, metrics-subsystem §2.1/§2.2).
...
- **AC5** — `/flow` schreibt den numerischen Anteil der Story-ID als int (`S-014` → `14`) als `item`-Schlüssel in `dispatches.jsonl` und `items.jsonl` (statt Issue-Nummer); alle übrigen Metrik-Felder unverändert. *(V5)*
...
```
dispatches.jsonl: { item: 14, … }     # S-014 → 14 (int); statt item: 123 (Issue-Nr.)
items.jsonl:      { item: 14, … }     # S-014 → 14 (int)
```
