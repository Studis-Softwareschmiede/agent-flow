...
- Der sofortige Push von Schritt 3 wird abgelehnt (non-fast-forward — eine andere Session hat zuerst gepusht). `/flow` fetcht/rebased und liest X erneut. X ist jetzt `In Progress` mit **fremdem** `claimed_by` und **frischem** `claimed_at` (nicht stale) → `/flow` verwirft die eigene lokale Reservierung, dispatcht **keinen** `coder` auf X und ruft erneut `board next` für das nächste Item auf.

### A2: Push abgelehnt, Grund war ein fremder, unbezogener Commit
- Push abgelehnt, aber nach Re-Read ist X weiterhin `To Do` (die Ablehnung kam von einem unbezogenen Board-Commit). `/flow` rebased und wiederholt den Claim-Push für X (begrenzte Anzahl Versuche, siehe Edge-Cases).
...
- **AC2** — Atomarität über Push: die Serialisierung nutzt die atomare `git push`-Semantik auf `<default_branch>`. Wird der Push abgelehnt (non-fast-forward), fetcht/rebased `/flow` und liest die Story erneut, bevor es weiter entscheidet. *(deckt A1/A2)*
...
- **AC4** — Retry bei unbezogener Ablehnung: ist die Story nach Re-Read weiterhin `To Do` (Ablehnung durch fremden, unbezogenen Commit), wiederholt `/flow` den Claim-Push nach Rebase — begrenzt durch ein festes Retry-Budget. *(deckt A2)*
...
3. Commit + `git push origin HEAD:<default_branch>`.
   - Erfolg → Bau (§3).
   - Abgelehnt → fetch/rebase, Re-Read: fremd-frisch → verwerfen + `board next` erneut (AC3); noch `To Do` → Retry (AC4, Budget); Budget erschöpft → Abbruch (AC13).
