...
- **Reserve-Operation (atomar):** `origin/<default_branch>` fetchen → nächsten freien Block berechnen (first-fit ab 1): jede `active`-Reservierung blockiert ihren **gesamten** `[range_start,range_end]`-Bereich (AC3 — kein Wert wird von zwei `active`-Reservierungen desselben Namespace geteilt); jede `released`-Reservierung blockiert **nur** ihren tatsächlich genutzten Teil `[range_start,high_water]` (`high_water: null` oder `< range_start` ⇒ nichts blockiert — der komplette Bereich ist frei wiederverwendbar, AC10) → Eintrag anhängen → committen → pushen. Push-Reject → re-fetch + neu berechnen + retry (begrenzt).
- **Konsument-Vertrag:** `coder`/`dba` lesen den für das laufende Feature reservierten Block (Ledger oder injiziertes Dossier) und vergeben strikt daraus (AC4); jede vergebene ID wird per `scripts/board-id-reserve.sh consume <namespace> <feature-or-story-id> <n>` im Ledger als High-Water-Mark vermerkt (hält AC10 ohne zusätzlichen Scan-Schritt korrekt).
...
- **Solo-Pfad-Freigabe (AC9/AC10-Parität):** im board-weiten `/flow`-Einzellauf (ohne `--parent`) reserviert `coder` lazy je Story (AC9, Scope-Schlüssel = eigene `S-###`, s. `agents/coder.md`); da es hier kein Batch-Ende gibt, das die Freigabe bündeln könnte, ruft `/flow` diese nach erfolgreichem Landen der Story (`Rollout-Gate: PASS`) **je Story** auf (`release <story-id>`) — idempotent/gefahrlos auch ohne vorherige Reservierung. Ohne diesen Schritt bliebe jede Solo-Reservierung dauerhaft `active`, statt wie in AC10 vorgesehen freigegeben zu werden.
- **Mechanik-Referenz:** alle Operationen (`reserve`/`extend`/`consume`/`release`/`seed`/`show`) sind Verben von `scripts/board-id-reserve.sh` — reine deterministische Bash/Python-Mechanik (NFR „token-arm"), arbeitet in einem isolierten `git worktree` gegen `origin/<default_branch>` und rührt den Working-Tree des Aufrufers nie an.
...
- **Bestehende, historisch vergebene IDs unterhalb des ersten Blocks:** die Block-Berechnung startet hinter dem höchsten bereits **im Bestand** vergebenen Wert je Namespace (kein Re-Use bestehender IDs). Mechanik: `scripts/board-id-reserve.sh seed <namespace> <höchste-bestehende-id>` legt einmalig eine **permanente** (nie `released`-bare) Reservierung mit `feature_id: _bestand` an, die `[1,<höchste-id>]` dauerhaft blockiert — nötig nur beim erstmaligen Adoptieren dieses Mechanismus in einem Projekt mit bereits existierenden nummerierten Artefakten.
