...
- **AC2** — Die Werte tragen feste Bedeutung: `draft` = in Arbeit, gilt noch nicht; `active` = verbindlich/in Kraft; `superseded` = durch eine neuere Spec ersetzt. Der Pfad ist `draft → active → superseded` (eine Spec startet als `draft`, wird `active`, wenn sie in Kraft tritt, und `superseded`, sobald eine neuere Spec sie ablöst). Der `draft`-Zustand ist eine **mögliche**, keine zwingend zu durchlaufende Persistenz-Phase: von `requirement` bei der Story-Anlage neu erzeugte, story-referenzierte Specs werden direkt auf `active` gestempelt (die `draft`-Phase wird übersprungen — Details `[[spec-auto-activation]]`); der `draft`-Default der Vorlage gilt für von Hand begonnene WIP-Specs.
