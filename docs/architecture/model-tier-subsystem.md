...
| `max-quality` | Token verfügbar, „richtig guter" Review/Test/Retro | Qualitäts-kritische Rollen auf das teuerste Modell |

Kurzformen (normalisiert): `low` → `low-cost`, `max`/`high` → `max-quality`, `mid`/`std` → `balanced`. Unbekannter Wert → `balanced` + einzeiliger Hinweis (nie raten).
...
- **Invariante I2:** je Rolle gilt `low-cost ≤ balanced ≤ max-quality` in der Kosten-/Fähigkeitsordnung (`haiku < sonnet < opus`) — ein Modus dreht nie *gegen* seine Richtung.
...
- Der `model`-Parameter des Task-Tools nimmt `opus | sonnet | haiku`. Präzedenz: **Override > Frontmatter > Session-Erbe**.
...
- `.claude/profile.md` trägt `cost_mode: balanced` (Default). Enum: `low-cost | balanced | max-quality`.
