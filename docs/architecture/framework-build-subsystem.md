...
1. **Frequenz-Schwelle.** Ein Pattern muss in **≥2 verschiedenen Projekten** UND **≥2 verschiedenen Code-Stellen** vorkommen, bevor `retro` es in einen Pack promoten darf.
   ...
   **Owner-Override (eng begrenzte Ausnahme, HART, Amendment 2026-07-18 — KEINE Aufweichung von G1).** Ist die „≥2 Projekte"-Schwelle **strukturell unerfüllbar** (das Pattern lebt in der einzigen Projektklasse ihrer Art in der Org — z.B. das einzige Postgres-Projekt), kann `retro` einen Owner-Override beanspruchen, aber **nur** wenn **alle vier** Bedingungen belegt sind: **(a)** die strukturelle Unerfüllbarkeit ist begründet (bloße Neuheit oder „noch kein Zweitprojekt aufgetaucht" genügt **nicht**); **(b)** **≥4 unabhängige Belegstellen in einem Projekt** (Datei/Zeile oder PR-Nummer, namentlich gelistet — strenger als die reguläre „≥2 Stellen"); **(c)** ein **explizites, datiertes** Owner-Approval mit auffindbarer Referenz (kein „mündliches"/implizites Approval); **(d)** ein standardisierter PR-Body-Abschnitt „Owner-Approved G1-Override" (kanonische Vorlage: `docs/specs/retro-g1-owner-override.md` Abschnitt „Verträge") **und** eine `LEARNINGS.md`-Kennzeichnung „Owner-Approved G1-Override". Fehlt ≥1 Bedingung, bleibt es beim regulären G1-Gate — `retro/G1-Violation` (Critical). Der Override ändert nur, **welche** Bedingung das reviewer-Gate (Schutzgitter #4) bei einem beanspruchten Override prüft, nicht **ob** es prüft — kein Bypass des Gates selbst. **Erster Register-Eintrag:** `agent-flow#335` (Regel `alembic/B01`, ki-investment als org-weit einziges Postgres-Projekt, 6 unabhängige Belegstellen, Owner-Approval 2026-07-14, `LEARNINGS.md`). Vollständige Vertrags-Spec: `docs/specs/retro-g1-owner-override.md` (AC1–AC7). **Unberührt:** `G1-Sonar` (§9a/H3) und die Estimator-G1-Freistellung (Modus E) — dieser Override gilt ausschliesslich für den Lessons-Pfad (Modus A).
...
3. **Cooldown:** retro läuft max. 1× pro `retro_cooldown_days`-Zeitraum pro Repo (optionales Profil-Feld `retro_cooldown_days` in `.claude/profile.md`, Ganzzahl ≥ 0 Tage; fehlt das Feld oder ist es leer/unparsbar ⇒ Default **1 Tag**; `0` = kein Cooldown, Stempel wird trotzdem geschrieben) — oder per explizitem `/retro --force`-Trigger. Persistiert in der projekt-lokalen Datei `.claude/lessons/.retro-last-run` (ISO-Datum). **Amendment (2026-05-31):** ursprünglich war ein Lookup in `LEARNINGS.md`-Spalte „Datum" geplant; ersetzt durch dedizierte Datei, weil ein leerer retro-Lauf (kein Pattern reif für Promotion) keinen Ledger-Eintrag erzeugt — eine Cooldown-Quelle aus dem Ledger wäre für solche Läufe blind. Die `.retro-last-run`-Datei tickt deterministisch bei jedem Lauf. **Amendment (2026-06-15, Persistenz-Garantie):** der kanonische State-Ort ist `<projekt-repo>/.claude/lessons/.retro-last-run` — **im geharvesteten Projekt-Repo** (cwd/`REPO_ROOT`), NICHT im agent-flow-PR-Ziel. Weil retro-Läufe isoliert ablaufen können (Lessons-Read aus Worktree, Pack-PR via `mktemp`-Klon gegen agent-flow), muss der Stempel nach jedem erfolgreichen Lauf committet+gepusht werden (`origin/<default_branch>` des Projekt-Repos, gemäss `merge_policy`), über denselben Commit-Pfad wie `baseline.json` (C4 in `agents/retro.md`). Ein stilles Verwerfen des Stempels in einem flüchtigen Working-Tree macht G3 wirkungslos (realer Ausfall: PR #140 + #155). Fehlt der Stempel / ist leer / unparsbar → kein Cooldown, Lauf erlaubt. Idempotenz: gleicher Tag → kein Leer-/Doppel-Commit. Robustheit: Push-Fehler bricht den Lauf NICHT ab, wird aber sichtbar gemeldet. **Amendment (2026-07-18, konfigurierbare Schwelle):** die bisherige fixe 7-Tage-Schwelle entfällt als Hardcode und wird durch das optionale Profil-Feld `retro_cooldown_days` ersetzt (Default **1** statt bisher fix 7; `0` = kein Cooldown). Owner-Entscheid 2026-07-18: in der Aufbauphase der Fabrik soll retro öfter laufen dürfen als 1×/Woche. Stempel-Ort, Persistenz-Mechanik (obige Amendments) und `--force`-Semantik bleiben unverändert. Vollständige Vertrags-Spec (Schwelle): `docs/specs/retro-cooldown-configurable.md` (AC1–AC6). Vollständige Vertrags-Spec (Persistenz): `docs/specs/retro-cooldown-persistence.md` (AC1–AC4, AC6–AC8).
4. **Reviewer-Gate bleibt.** Der retro-PR durchläuft denselben reviewer-Loop wie jeder andere PR — kein Bypass, kein Merge ohne `PASS`. Cross-Pack-Promotions (eine retro-Welle, die mehrere Packs trifft) werden gebündelt als **ein PR mit mehreren Regeln** (kein PR-Spam). **Amendment (2026-07-18, Auto-Merge):** `Review-Gate: PASS` merged den retro-PR seither automatisch (squash, kein Mensch-Approve mehr — Owner-Entscheid, `docs/specs/retro-auto-merge.md`); `CHANGES-REQUIRED` bleibt unmerged (Fix-Loop, max. 3 Iterationen, danach offen + Meldung). Gilt nur für `retro`; `train`-PRs bleiben beim bisherigen Gate (reviewer-Check + Mensch-Approve).
