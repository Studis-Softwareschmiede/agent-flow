...
1. **Frequenz-Schwelle.** Ein Pattern muss in **≥2 verschiedenen Projekten** UND **≥2 verschiedenen Code-Stellen** vorkommen, bevor `retro` es in einen Pack promoten darf.
   ...
   **Verfall der `Proposed`-Einträge (HART, Amendment 2026-06-10).** Damit das Register nicht monoton mit Eintagsfliegen wächst, trägt jede `Proposed`-Zeile ein **`expires`-Datum = Eintrags-/letztes-Sichtungs-Datum + 6 Monate** (Status-Suffix `Proposed · expires <iso>`). Regeln:
   - **Refresh bei Wiedersichtung:** taucht dasselbe Pattern erneut auf (auch im *selben* Repo, was G1 nicht erfüllt), setzt `retro` `expires` neu auf +6 Monate. Stetig wiederkehrende Patterns verfallen damit nie versehentlich.
   ...
   - **6 Monate** ist der Default: lang genug, dass ein real wiederkehrendes Pattern ein Zweit-Repo trifft; kurz genug, dass Sprach-/Framework-Lektionen nicht über Versionssprünge hinweg „tot" weiterleben.
