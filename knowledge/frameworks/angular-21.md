...
- `angular-21/A09` — **`NgModuleFactory` entfernt (Breaking, since 21).** `NgModuleFactory` ist in v21 vollständig entfernt — Code, der `NgModuleFactory` direkt instanziiert oder referenziert, bricht beim Build. Migrationspfad: `NgModule` direkt verwenden (kein Factory-Pattern mehr nötig). Häufig betroffen: ältere Lazy-Loading-Patterns und custom Compiler-Setups, die `NgModuleFactory`-Typen trugen. [src: https://github.com/angular/angular/releases/tag/21.0.0, since: 21.0]
...
- Entfernte APIs (A04, A09) und Host-Binding-Type-Checking (A05) sind häufige Build-Brecher beim Upgrade — zuerst dort suchen, wenn der Build nach einem Major-Bump rot ist. `emitDeclarationOnly: true` in tsconfig → sofort entfernen (A10). `router.lastSuccessfulNavigation` — ist jetzt ein Signal, Invocation nötig (A11).
