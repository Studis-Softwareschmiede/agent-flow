...
Spring-Boot 4.x (Major-Range `>=4.0, <5.0`, GA 2025-11-20). Geladen bei `profile.frameworks` enthält `spring-boot@4`. Regel-IDs: `spring-boot-4/A<NN>` (Sektion A, train) · `spring-boot-4/B<NN>` (Sektion B, retro) · `spring-boot-4/C<NN>` (Sektion C, Floor).
...
- Jackson-2-Annotationen (`@JsonComponent`/`@JsonMixin`) oder `com.fasterxml.jackson`-Group ohne `spring-boot-jackson2`-Begründung → **Important** (A05, Jackson 3 ist Default).
...
- Build via Maven (`mvn -B -ntp verify`, ≥3.6.3) oder Gradle (`./gradlew build --no-daemon`, ≥8.14) — siehe `knowledge/build/<build>.md`.
