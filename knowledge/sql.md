...
- `sql/R12` — **PostgreSQL 18 (stable seit Sep 2025)**: Eine generierte Spalte (`GENERATED ALWAYS AS (...)`) ist **per Default virtuell**, wenn `STORED`/`VIRTUAL` nicht explizit angegeben wird — Berechnung bei jedem Lesen (wie eine View), kein Speicherplatz (Zitat: „A generated column is by default of the virtual kind. Use the keywords VIRTUAL or STORED to make the choice explicit."). Virtuelle Spalten können **nicht indiziert** werden und werden **nicht** logisch repliziert (`publish_generated_columns` deckt nur `STORED`). Für indizierte/replizierte generierte Spalten weiterhin explizit `GENERATED ALWAYS AS (...) STORED` schreiben. Quelle: [PG18 Generated Columns Docs](https://www.postgresql.org/docs/18/ddl-generated-columns.html)
- `sql/R13` — **PostgreSQL 18 (stable seit Sep 2025) — Verhaltensänderung**: `AFTER`-Trigger laufen jetzt mit der Rolle, die beim **Queuing** des Trigger-Events aktiv war — nicht mehr mit der Rolle, die bei Trigger-**Ausführung** (z.B. bei `COMMIT`) aktiv ist (Zitat: „Previously such triggers were run as the role that was active at trigger execution time (e.g., at COMMIT). This is significant for cases where the role is changed between queue time and transaction commit."). Audit-/Enforcement-Trigger, die einen Rollenwechsel (`SET ROLE`/`SET LOCAL ROLE`) zwischen Queue-Zeit und Commit voraussetzen, müssen auf PG18 neu verifiziert werden. Quelle: [PG18 Release Notes — AFTER Triggers](https://www.postgresql.org/docs/18/release-18.html)
...
- Generierte Spalte mit Index-/Replikations-Anspruch aber ohne explizites `STORED` (PG 18+, Default ist virtuell) → **Important** (`sql/R12`).
- Audit-/Enforcement-`AFTER`-Trigger, der einen Rollenwechsel zwischen Queue-Zeit und Commit voraussetzt (PG 18+ Verhaltensänderung) → **Important** (`sql/R13`); Trigger-Logik neu verifizieren.
