...
- `mongo/R06` — **`{field: null}`-Queries matchen ab MongoDB 8.0 keine `undefined`-Werte mehr (Breaking Change)**: Vor 8.0 matchte `db.col.find({name: null})` auch Dokumente, bei denen `name` den BSON-Typ `undefined` hatte. Ab MongoDB 8.0 matcht `null` ausschliesslich fehlende Felder und explizite `null`-Werte — `undefined` wird separat behandelt. Der BSON-Typ `undefined` ist in 8.0 zusätzlich deprecated (mongosh wandelt JS-`undefined` bei Inserts still zu `null`). Mitigierung: bestehende Collections auf `undefined`-Werte prüfen (`db.col.find({field: {$type: 6}})`) und mit `$set: {field: null}` normalisieren, bevor auf 8.0 migriert wird. Quelle: [MongoDB 8.0 Compatibility Changes — Null Query Behavior](https://www.mongodb.com/docs/manual/release-notes/8.0-compatibility/) · [Migrate Undefined Data](https://www.mongodb.com/docs/manual/reference/bson-types/migrate-undefined/)

- `mongo/R07` — **Server-seitiges JavaScript deprecated seit MongoDB 8.0**: `$function`, `$accumulator` und `$where` sind in MongoDB 8.0 deprecated — der Server loggt eine Warnung bei jeder Ausführung. Alternativen: `$where` → `$expr` mit nativen Aggregations-Operatoren ersetzen; `$function`/`$accumulator` → native Pipeline-Stages (`$reduce`, `$map`, `$filter`, `$mergeObjects`, `$group` mit eingebauten Akkumulatoren). Server-seitiges JavaScript behindert Sharding-Performance (Single-threaded Ausführung), verhindert Query-Plan-Caching und ist ein Sicherheitsrisiko. Abschaltung serverseits via `security.javascriptEnabled: false` empfohlen, sobald keine dieser Operatoren mehr genutzt werden. Quelle: [MongoDB Manual — Server-side JavaScript](https://www.mongodb.com/docs/manual/core/server-side-javascript/) · [MongoDB 8.0 Release Notes](https://www.mongodb.com/docs/manual/release-notes/8.0/)
