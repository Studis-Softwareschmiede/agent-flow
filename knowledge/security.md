...
> **last_trained:** 2026-07-21 — Frische-Signal für durable Sicherheits-Prinzipien. `train` setzt das Datum bei jedem `/train security` auf heute; `/flow` nudged, wenn es > 90 Tage her ist.
...
- `security/R19` — **PQC-Migrationsfristen konkretisiert** (NIST IR 8547, Initial Public Draft, Nov. 2024 — schärft `security/R12`): klassische Public-Key-Algorithmen haben ein festes Enddatum, nicht nur eine vage "vorerst sicher"-Aussage. RSA und ECDSA mit **112 Bit Sicherheitsstärke**: **deprecated ab 2030**, **disallowed ab 2035**. **Auch ≥128-Bit-Varianten** (RSA, ECDSA, EdDSA, Finite-Field-/Elliptic-Curve-DH und -MQV) werden **2035 disallowed** — das Enddatum gilt unabhängig von der Schlüssellänge, nicht nur für die schwachen 112-Bit-Parameter. National Security Memorandum 10 (NSM-10) setzt 2035 als Ziel für die vollständige PQC-Migration der US-Bundessysteme. Praxis-Konsequenz: langlebige Zertifikate/Signatur-Schemata (Code-Signing, Archiv-Signaturen, TLS-Root-Rollout-Pläne), deren geplante Nutzungsdauer über 2035 hinausreicht, brauchen jetzt einen dokumentierten PQC-Migrationspfad. → [NIST IR 8547 (ipd)](https://nvlpubs.nist.gov/nistpubs/ir/2024/NIST.IR.8547.ipd.pdf) Table 2 (S.13, Digital Signatures) / Table 4 (S.14, Key Establishment)
