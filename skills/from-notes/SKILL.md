...
1. **Übersetzen (niedrigste Nachfrage-Schwelle, AC10):** Aus `$CORPUS_FILE` das Konzept nach `docs/concept.md` schreiben (Skelett `templates/_docs/concept.md`; existiert bereits ein Root-`CONCEPT.md`-Layout wie in manchen Projekten, dort die betroffenen Sektionen ergänzen statt einer neuen Datei — dieselbe Layout-Konvention wie `reconcile`). **Jede** relevante Mehrdeutigkeit/jeder Widerspruch im Ideen-Text wird zur **Frage** (Fragenkatalog-Eintrag), **nicht** zur stillen Annahme — die Schwelle ist hier bewusst niedrig (AC10). Jeder Katalog-Eintrag trägt im `quelle`-Feld den Herkunfts-Marker der Quellnotiz. **Klassifizierte PM-Notizen (0c), nur für die in dieser Erweiterung tatsächlich implementierten Ziel-Zeilen ausgenommen:** ausschliesslich für die **fünf** in Stufe b §2.1 bereits angewandten Mapping-Zeilen — `problem-statement`/`prd`: Problem Statement (+ Goals) → Zweck, `prd`: Non-Goals → Nicht-Ziele, `prd`: Scope → Verträge/Abhängigkeiten, `user-stories`/`prd`: User Stories → Main Success Scenario/Alternative Flows, `edge-cases`/`prd`: Edge Cases → Edge-Cases & Fehlerverhalten — fliesst der Inhalt **nicht** in diesen Übersetzungsschritt, sondern direkt in die Spec (Stufe b, §2.1). **Alle übrigen** klassifizierten Notizen/Sektionen — `hypothesis`, `acceptance-criteria`/`prd`: Functional Requirements (`FR-n`), `adr`, `launch-checklist`/`prd`: Milestones sowie `prd`: Risiken/Success Metrics/Open Questions — haben **in dieser Story noch kein** implementiertes Mapping-Ziel (das folgt erst mit AC3/AC4/AC5/AC8/AC9, Folge-Story): sie durchlaufen bis dahin **unverändert** diesen Übersetzungsschritt wie unklassifizierte Notizen (landen also vorerst in `docs/concept.md`, inkl. der normalen Fragenkatalog-Behandlung bei Mehrdeutigkeit) — **kein** stilles Weglassen, nur ein noch nicht gezogenes feingranulares Ziel. Nur **unklassifizierte** Notizen laufen ohnehin wie bisher unverändert.
...
1. **Spec(s) schreiben:** je Capability eine `docs/specs/<feature-slug>.md` aus `templates/_docs/specs/_template.md` — Zweck, Verhalten, **nummerierte Acceptance-Kriterien (AC1, AC2, …)**, Verträge, Edge-Cases, NFRs, Nicht-Ziele. **`spec_format`-Stempel:** den Wert 1:1 aus der **aktuellen** `_template.md` übernehmen (nicht hartkodieren), wie es der Spec-Vertrag (`docs/specs/spec-format-field.md` AC3) und `requirement` fordern. **Konzept-Herkunft:** jede Spec referenziert ihre `C-NNN`-Herkunft `(← C-NNN)` (AC16).
   - **Mapping-Dispatch für klassifizierte PM-Notizen (0c, `docs/specs/pm-import.md` AC2):** für jede in 0c klassifizierte Notiz bzw. `prd`-Sektion die **Mapping-Tabelle** (`docs/specs/pm-import.md` → Verträge) nachschlagen und den Inhalt **deterministisch** an das dort genannte Ziel schreiben — gleicher Input erzeugt immer dieselbe Zuordnung, **keine** freie Neu-Interpretation von Inhalten mit definiertem Ziel. **Container (`prd`):** jede in 0c markierte Sektion wird **einzeln** nach ihrer eigenen Zeile behandelt (Sektionsebene), nicht die gesamte Notiz nach einer Zeile. In dieser Erweiterung angewandte Zeilen:
     ...
     - `user-stories` / `prd`: User Stories (`US-n`) → Spec «Main Success Scenario» + «Alternative Flows» (Story → Flow-Schritt, stabile `US-n` mitführen; Flows optional).
     - `edge-cases` / `prd`: Edge Cases → Spec «Edge-Cases & Fehlerverhalten» (1:1).
     - Die übrigen Tabellenzeilen (`hypothesis` → `BR-NNN`, `FR`/GWT → Acceptance-Kriterien, Risiken/Success-Metrics/Open-Questions, `launch-checklist`, `adr`, Idempotenz-Anker) tragen ihre feldgenaue Mechanik unter AC3/AC4/AC5/AC8/AC9/AC6 — deren Anwendung folgt als Erweiterung dieser Sektion; bis dahin **nicht** ad hoc nach dieser Zeile interpretieren (Determinismus vor Eleganz, NFR).
...
4. **Commit Stufe b (AC12):** Erst nach beantwortetem/leerem Katalog:
   ...
   Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>" && git push
   ```
   Alle in dieser Stufe berührten `docs/`-Dateien (Spec(s) + ggf. `architecture*.md`/`data-model.md`/**`design.md` bei UI-Projekten**) fahren in **einem** Stufe-b-Commit (`git add docs/` erfasst `docs/design.md` automatisch mit). Branch-Protection → docs-only-PR + Self-Merge.
