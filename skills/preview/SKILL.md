...
- Als Liste rendern: `<name>` → ladbar mit `/agent-flow:preview up <name-lowercase>` (Image-/`up`-Name **immer lowercase**, auch wenn das Repo Großbuchstaben hat — sonst schlägt der `docker pull` fehl); bereits laufende via `list` markieren.
