...
- **Start-`board/areas.yaml` scaffolden** (board-areas Spec AC1; new-project-board Spec V8/AC8) — **nur wenn kein File-Board-Skelett bereits vorhanden ist**:
  - Prüfen ob `board/areas.yaml` bereits existiert — falls ja: keine Änderung (idempotent).
  - Falls `board/` bereits existiert (neu angelegtes File-Board): **Primärpfad — Bereiche aus `docs/concept.md` ableiten:** Den gerade erstellten Konzept-Entwurf lesen und Produktbereiche/Kacheln extrahieren. Typische Signale: Abschnitt-Header (§ 2–5), Bullet-Listen unter Überschriften wie „Bereiche", „Module", „Funktionsgruppen", oder Spalten in Datenmodel-Tabellen. Pro erkanntem Bereich: `id` kebab-case aus Bereichsnamen normalisiert, `titel` aus der Überschrift/Bullet-Überschrift, `beschreibung` aus dem einleitenden Satz des Abschnitts (genau 1 Satz, max. ~80 Zeichen), `reihenfolge` aufsteigend von 1. Feldformat strikt nach [[board-areas]] AC1. — **Fallback (kein Bereich aus Konzept ableitbar):** minimale `board/areas.yaml` mit Platzhalter-Bereich `id: kern` schreiben und dokumentieren: „Aus `docs/concept.md`-Entwurf kein Bereich ableitbar; Bereiche später ergänzbar per manuellem Edit von `board/areas.yaml`".
  - Falls `board/` nicht existiert (alte GitHub-Project-Repos oder Repos ohne Board): Schritt überspringen + dokumentieren (→ Backlog-Item „File-Board-Skelett bei nächster Gelegenheit ergänzen + `board/areas.yaml` aus `docs/concept.md` ableiten").
