...
- Fehlende `Dockerfile` / `.github/workflows/build.yml` / `security.yml` / `.github/dependabot.yml` aus `${CLAUDE_PLUGIN_ROOT}/templates/` ergänzen (Sprach-Ökosystem im dependabot.yml setzen). **Bei `profile.language` = `html`/`flutter`/`angular` zusätzlich fehlendes `nginx.conf`** aus demselben Template-Ordner ergänzen — das Dockerfile kopiert sie explizit (`COPY nginx.conf …`), ohne sie bricht `docker build` (Build-Versionierung, `docs/specs/build-version-stamping.md` AC6/AC7).
...
## 2i. Admin-Bereich-Scaffolding (Spec [`docs/specs/admin-bereich-scaffolding.md`](../../docs/specs/admin-bereich-scaffolding.md) AC5, Konzept [`docs/architecture/admin-bereich-subsystem.md`](../../docs/architecture/admin-bereich-subsystem.md))

Läuft **nach** dem Regressions-Scaffolding (2h) und **vor** dem Audit (3) — **nur bei UI-Projekten** (dieselbe Erkennung wie `new-project` Schritt 4b / [`docs/specs/design-owner-approval.md`](../../docs/specs/design-owner-approval.md) AC3/„UI-Projekt-Erkennung": `profile.language` ∈ `flutter|angular|html` — bei Array-Form genügt ein Treffer — **oder** `profile.domains` enthält `ui`/`accessibility`). Nicht-UI-Repos: Schritt komplett überspringen — **kein** Scaffold, **keine** Board-Story (AC6).

**a) Spec-Vorlage** — existiert `docs/specs/admin-bereich.md` bereits → **nicht anfassen** (bestehenden Stand behalten). Fehlt sie → kopieren: `cp ${CLAUDE_PLUGIN_ROOT}/templates/_docs/specs/admin-bereich.md docs/specs/admin-bereich.md`.

**b) Manifest-Vorlage** — existiert `config/admin-manifest.yaml` bereits → **nicht anfassen**. Fehlt sie → kopieren: `mkdir -p config && cp ${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/admin-manifest.yaml config/admin-manifest.yaml`.

**c) `set-admin-password.sh`** — existiert `scripts/set-admin-password.sh` bereits → **nicht anfassen**. Fehlt es → kopieren: `cp ${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/set-admin-password.sh scripts/set-admin-password.sh && chmod +x scripts/set-admin-password.sh`.

**d) Board-Story „Admin-Bereich" idempotent anlegen** (Titel-/Spec-Match, nie doppelt — AC5-Idempotenz):
- Feature-Parent: existiert bereits ein Feature mit Titel „Admin-Bereich" (`board list --type feature`) → dessen `F-###` verwenden; sonst neu anlegen: `board feature add --title "Admin-Bereich" --goal "Passwortgeschützter Admin-Bereich mit deklarativem Config-Editor (Fabrik-Standard, docs/architecture/admin-bereich-subsystem.md)" --priority P2`.
- Story: existiert bereits eine Story mit Titel „Admin-Bereich" **und** `spec: docs/specs/admin-bereich.md` unter diesem Parent (`board list --type story --parent <F-###>`) → **nichts tun**. Sonst: `board story add --parent <F-###> --title "Admin-Bereich" --spec docs/specs/admin-bereich.md --implements AC1,AC2,AC3,AC4,AC5,AC6,AC7,AC8,AC9,AC10,AC11 --priority P2`.

**e) Kein Auto-Fix.** Identisch zu 2g/2h: Schritt 2i schreibt/ergänzt nur die oben genannten Scaffold-Artefakte + die eine idempotente Story; bestehende Admin-Dateien werden **nie** überschrieben. **Cross-Wellen-Graceful-Degradation** (admin-bereich-subsystem.md §8): existiert `${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/` (noch) nicht → ⚠ Konsolen-Warnung, Schritt überspringen statt zu scheitern.

## 2j. Versions-Endpunkt-Scaffolding (Spec [`docs/specs/build-version-stamping.md`](../../docs/specs/build-version-stamping.md) AC8)

Läuft **nach** dem Admin-Bereich-Scaffolding (2i) und **vor** dem Audit (3) — **nur bei Service-Projekten** (`profile.language` ∈ `js|java|python` — bei Array-Form genügt ein Treffer; identisch zur Service-Dockerfile-Menge aus `docs/specs/build-version-stamping.md` AC2, keine zweite Definition). **Frontend-Repos** (`html`/`flutter`/`angular`): Schritt komplett überspringen — **kein** Scaffold, **keine** Board-Story; dort reicht das statische `version.json`-Stempel-Artefakt aus dem Build-Versionierungs-Muster (AC6).

**a) Spec-Vorlage** — existiert `docs/specs/version-endpoint.md` bereits → **nicht anfassen** (bestehenden Stand behalten). Fehlt sie → kopieren: `cp ${CLAUDE_PLUGIN_ROOT}/templates/_docs/specs/version-endpoint.md docs/specs/version-endpoint.md`.

**b) Board-Story „Version-Endpunkt" idempotent anlegen** (Titel-/Spec-Match, nie doppelt):
- Feature-Parent: existiert bereits ein Feature mit Titel „Build-Versionierung" (`board list --type feature`) → dessen `F-###` verwenden; sonst neu anlegen: `board feature add --title "Build-Versionierung" --goal "Robuste, konsistente Build-Versionierung (Fabrik-Standard, docs/specs/build-version-stamping.md)" --priority P2`.
- Story: existiert bereits eine Story mit Titel „Version-Endpunkt" **und** `spec: docs/specs/version-endpoint.md` unter diesem Parent (`board list --type story --parent <F-###>`) → **nichts tun**. Sonst: `board story add --parent <F-###> --title "Version-Endpunkt" --spec docs/specs/version-endpoint.md --implements AC1,AC2,AC3,AC4 --priority P2`.

**c) Kein Auto-Fix.** Identisch zu 2g/2h/2i: Schritt 2j schreibt/ergänzt nur die Spec-Vorlage + die eine idempotente Story; eine bestehende `docs/specs/version-endpoint.md` wird **nie** überschrieben. Die stack-spezifische Route-Implementierung ist explizites Nicht-Ziel dieses Schritts — sie entsteht projekt-lokal über `/flow`.
...
- **Admin-Bereich-Scaffolding (Schritt 2i) läuft nur bei UI-Projekten** (Spec `docs/specs/admin-bereich-scaffolding.md` AC5/AC6) und ist strikt idempotent — bestehende `docs/specs/admin-bereich.md`/`config/admin-manifest.yaml`/`scripts/set-admin-password.sh` werden nie überschrieben, die Board-Story „Admin-Bereich" wird nie doppelt angelegt (Titel-/Spec-Match). Die stack-spezifische Implementierung (Routen/Session/UI-Rendering) ist explizites Nicht-Ziel dieses Schritts (AC8) — sie entsteht projekt-lokal über `/flow`.
- **Versions-Endpunkt-Scaffolding (Schritt 2j) läuft nur bei Service-Projekten** (Spec `docs/specs/build-version-stamping.md` AC8) und ist strikt idempotent — eine bestehende `docs/specs/version-endpoint.md` wird nie überschrieben, die Board-Story „Version-Endpunkt" wird nie doppelt angelegt (Titel-/Spec-Match). Die stack-spezifische Route-Implementierung ist explizites Nicht-Ziel dieses Schritts — sie entsteht projekt-lokal über `/flow`.
