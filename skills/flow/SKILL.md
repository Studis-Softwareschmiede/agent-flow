...
- **Orchestrator-Lessons lesen (einmal, zu Beginn):** existiert `.claude/lessons/flow.md` im Projekt-Repo, diese Datei lesen und ihre Regeln für den gesamten Lauf befolgen — der **kanonische** Tier-1-Kanal für die Orchestrator-Ebene (Landen/Konsolidieren/Recovery/Dispatch-Ökonomie), analog zu den coder/reviewer/tester-Lessons, die die Arbeits-Agenten über ihre Verträge lesen. Fehlt die Datei → kein Fehler, weiter. Der Orchestrator prependet eigene neue Lessons dorthin (newest-first) und landet sie mit dem nächsten Board-Commit. `flow.md` dient dabei **doppelt**: nicht nur als Vorlauf für den nächsten `/flow`-Lauf, sondern auch als Fabrik-Lernquelle — `retro` **harvestet** ihn gleichrangig zu den coder/reviewer/tester-Lessons (Verweis: S-113/AC1). *[seen-in: ki-investment 2026-07-14 — Orchestrator-Lessons (Resume-Kosten 381k Token, Feature-Konsolidierung) existierten persistent in flow.md, wurden aber von keinem frischen /flow-Lauf gelesen, weil kein Vertrag darauf zeigte; promoted: 2026-07-14]*
- **Orchestrator-Lessons schreiben (jeder Orchestrator, vor Session-/Lauf-Ende):** Session-Lessons nach `.claude/lessons/flow.md` zu prependen ist Pflicht **nicht nur für `/flow`**, sondern für **jeden Orchestrator** — auch den **Nachtwächter-Außenlauf** und eine **Owner-Session, die substanzielle Mehr-Feature-Arbeit koordiniert** (auch über general-purpose-Subagenten). Vor Session-/Lauf-Ende die wichtigsten Session-Lessons dorthin prependen (newest-first), damit auch Groß-Feature-Arbeit für die Retro sichtbar wird und nicht nur Klein-Story-Arbeit der Agenten. *[seen-in: Retro 2026-07-21 — Groß-Feature-Arbeit hinterließ keine Lessons in flow.md, weil kein Vertrag den koordinierenden Orchestrator jenseits von `/flow` zum Schreiben verpflichtete; promoted: 2026-07-21]*
...
- **Security-Frische (einmaliger Nudge):** `last_trained:` aus `${CLAUDE_PLUGIN_ROOT}/knowledge/security.md` lesen; ist es **> 90 Tage** her → einmal ausgeben: „🔒 security-Pack ist <N> Tage alt — `/train security` erwägen. Betrifft nur die **durablen Sicherheits-Prinzipien** im Pack; tagesaktuelle CVEs/Exploits gehören NICHT hierher, sondern in die self-updating Ebene — Dependabot + geplanter Scanner-Lauf (Nuclei/ZAP-Feeds aktualisieren sich pro Lauf), s. `docs/architecture/red-team-subsystem.md` §4 + §5." (nur Hinweis, blockiert nicht).
...
- **Landung vs. lokale Nacharbeit sind unabhängig (flow/P3):** `board-ship.sh` kann am **lokalen Nachschritt** (`gh pr merge --delete-branch` / Board-Flip) mit `fatal: '<branch>' is already used by worktree …` (Zielbranch `main` bzw. `feature/<F>` in IRGENDEINEM anderen Worktree ausgecheckt) mit Exit ≠0 abbrechen — **obwohl der PR remote bereits MERGED ist**. Ein solcher Exit ≠0 ist deshalb NICHT automatisch ein „nicht gelandet"-Fall (§5-Default „Board bleibt `In Review`" trifft hier NICHT zu). Regel: vor der In-Review-/Fehler-Meldung IMMER den **Remote-State** prüfen (`gh pr list --head <branch> --state all`); state `MERGED` ⇒ die Landung ist erfolgt — nur die **Restschritte mechanisch nachziehen** (CI-Watch falls `main`, Board-Flip im frisch von `origin/<default_branch>` resetteten Detached-Checkout, Push), statt die Landung als gescheitert zu behandeln oder blind zu retryen (ein Re-Run trifft dieselbe Worktree-Belegung erneut). Prävention: **Story-Worktrees SOFORT nach bestätigter Landung abbauen** (SR1) — der Ship-Flip checkt den Zielbranch aus und hält ihn sonst belegt. *[seen-in: 2 Projekten (agent-flow board-ship.sh S-074/PR #357 + S-075/PR #359, ki-investment F-017-Drain Modus A/B), promoted: 2026-07-19]*
