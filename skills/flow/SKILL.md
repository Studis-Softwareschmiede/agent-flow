...
## 0. Setup
- `.claude/profile.md` lesen → Board-Referenz, `merge_policy` (`pr`|`direct`), Build/Test-Befehle, **`default_branch`**. Fehlt der Key (Alt-Repo) → einmalig zur Laufzeit ableiten und merken: `default_branch="$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name)"` (NICHT `main` annehmen — adoptierte Forks haben oft `master`).
