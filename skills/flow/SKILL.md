...
- **Post-Rebase-Verifikation (flow/P2):** Nach jeder Rebase- oder Konfliktauflösung — und bevor das Item auf `Done` gesetzt wird — MUSS der Orchestrator den vollen Test-Run gegen den **finalen main-Stand** bestätigen (nicht nur gegen den isolierten Worktree). Methode: nach dem Merge auf main `npm test` (bzw. das `profile.build`-Kommando) direkt auf main ausführen ODER explizit den `tester` gegen den post-merge `main`-Stand dispatchen. Ein Konfliktlöser, der „Tests grün" nur im Worktree-Kontext bestätigt, kann einen main-Stand mit roten Tests hinterlassen (umgeschriebene Tests kommen nicht sauber an / Mismatch Implementierung↔Test). *[seen-in: dev-gui-cloudflare Rebase nach Items #109/#110 (3 rote Tests auf main nach Konfliktauflösung); promoted: 2026-06-09]*
