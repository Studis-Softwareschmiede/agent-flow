...
Nach `tester`-PASS: `scripts/board-ship.sh <story-id> [<container-name>]` **direkt per Bash ausführen** (kein Task-Dispatch mehr für den SHIP-Pfad). Das Skript übernimmt git-Operationen (Merge + Push), CI-Beobachtung (mit `headSha`-Race-Schutz, cicd/F06), lokalen Rollout (pull + recreate, Rollout-Verifikation gegen die tatsächlich deployte Revision) sowie den Board-Flip auf `Done` (via `board set`, committet + gepusht) — **rein mechanisch, kein LLM-Urteil**. Exit 0 = gelandet; Exit ≠0 = Abbruch mit Klartext-Fehlermeldung, Board bleibt unverändert (`In Review`) — dem User melden, Session-Ende nach diesem Item (kein automatischer Retry).
