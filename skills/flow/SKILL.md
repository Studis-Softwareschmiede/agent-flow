...
- **`pr`:** Branch `item-<n>-<slug>` → commit (Message aus Item-Titel + coder-Summary) → push → PR öffnen → Item → **In Review**. Nach deinem Merge → **Done** (+ PR verlinkt).
  ...
  - **Sonar-Quality-Gate prüfen** (nur wenn `profile.sonar.edition == sonarcloud`; Detail `knowledge/quality/sonar.md` §4): nach dem PR-Open best-effort auf den `sonar.yml`-CI-Run warten (`gh run watch`, kurzes Timeout), dann das **New-Code-Gate** abrufen — `GET {profile.sonar.host_url}/api/qualitygates/project_status?projectKey=<key>&pullRequest=<n>` (Token: `SONAR_TOKEN`). `projectStatus.status == ERROR` → als `FINDINGS` zurück an den `coder` (zählt zum Schleifenschutz §3), **nicht** zum Merge empfehlen. `OK`/nicht abrufbar/Timeout → best-effort weiter (nicht blockieren). `sonarqube-ce` kann kein PR-Gate → übersprungen.
