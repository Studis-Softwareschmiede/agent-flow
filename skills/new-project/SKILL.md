...
4. **`.claude/` scaffolden** (aus `${CLAUDE_PLUGIN_ROOT}/templates/<lang>/`):
   ...
   - `CLAUDE.md`: minimaler Kontext (Template + 1–2 Fragen). **Pflicht-Block:** den Abschnitt „Kommunikation mit dem Owner" aus `${CLAUDE_PLUGIN_ROOT}/templates/_shared/owner-communication.md` unverändert übernehmen (Stil-Regel für die Owner-Session: kurz, wenig Jargon, 3-Schichten-Antwort, Steuerwörter `kurz`/`erklär`/`technisch`). Rest projekt-spezifisch.
