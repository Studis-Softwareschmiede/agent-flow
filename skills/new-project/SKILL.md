---
name: new-project
description: Bootstrappt ein Projekt der Softwareschmiede — legt Repo + board/-Skelett (File-Board) an, erkennt/erfragt den Stack + DB-Dialekt + optionale Companions (Cache/Queue/Sessions) + Build-Tool + optionale Frameworks, scaffoldet .claude/ (profile, CLAUDE.md, lessons, memory) + Dockerfile + CI + optionales DB-Compose-Fragment + optionale Companion-Fragmente + das Playwright-Regressions-Grundgerüst (tests/regression/, Config, .gitignore, Dev-Dependency) + bei UI-Projekten das Admin-Bereich-Scaffold (Spec-Vorlage, Manifest, set-admin-password.sh, idempotente Board-Story) aus ${CLAUDE_PLUGIN_ROOT}/templates/. /init adoptiert ein bestehendes Repo. Schreibt KEINEN App-Code.
---
...
4g. **Admin-Bereich-Scaffold** (Spec [`docs/specs/admin-bereich-scaffolding.md`](../../docs/specs/admin-bereich-scaffolding.md) AC4, Konzept [`docs/architecture/admin-bereich-subsystem.md`](../../docs/architecture/admin-bereich-subsystem.md)) — **nur bei UI-Projekten** (dieselbe Erkennung wie Schritt 4b: `profile.language` ∈ `flutter|angular|html` — bei Array-Form genügt ein Treffer — **oder** `profile.domains` enthält `ui`/`accessibility`; identisch zu [`docs/specs/design-owner-approval.md`](../../docs/specs/design-owner-approval.md) AC3/„UI-Projekt-Erkennung", keine zweite Definition). **Nicht-UI-Projekte:** Schritt komplett überspringen — **kein** Scaffold, **keine** Board-Story (AC6).
   1. **Spec-Vorlage kopieren** (idempotent — nur falls `docs/specs/admin-bereich.md` noch nicht existiert): `cp ${CLAUDE_PLUGIN_ROOT}/templates/_docs/specs/admin-bereich.md docs/specs/admin-bereich.md`.
   2. **Manifest-Vorlage kopieren** (idempotent — nur falls `config/admin-manifest.yaml` noch nicht existiert): `mkdir -p config && cp ${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/admin-manifest.yaml config/admin-manifest.yaml`.
   3. **`set-admin-password.sh` kopieren** (idempotent — nur falls `scripts/set-admin-password.sh` noch nicht existiert): `cp ${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/set-admin-password.sh scripts/set-admin-password.sh && chmod +x scripts/set-admin-password.sh`.
   4. **Board-Story „Admin-Bereich" idempotent anlegen** (Titel-/Spec-Match — nie doppelt, Edge-Case der Spec):
      ...
      - Story: existiert bereits eine Story mit Titel „Admin-Bereich" **und** `spec: docs/specs/admin-bereich.md` unter diesem Parent (`board list --type story --parent <F-###>`) → **nichts tun**. Sonst: `board story add --parent <F-###> --title "Admin-Bereich" --spec docs/specs/admin-bereich.md --implements AC1,AC2,AC3,AC4,AC5,AC6,AC7,AC8,AC9,AC10,AC11 --priority P2` (Status `To Do` per CLI-Default).
   5. **Cross-Wellen-Graceful-Degradation** (admin-bereich-subsystem.md §8): existiert `${CLAUDE_PLUGIN_ROOT}/templates/_shared/admin/` (noch) nicht → ⚠ Konsolen-Warnung ausgeben, Schritt 4g überspringen statt zu scheitern (analog Pack-Vorhandensein-Check in Schritt 4).

4h. **Versions-Endpunkt-Scaffold** (Spec [`docs/specs/build-version-stamping.md`](../../docs/specs/build-version-stamping.md) AC8) — **nur bei Service-Projekten** (`profile.language` ∈ `js|java|python` — bei Array-Form genügt ein Treffer; identisch zur Service-Dockerfile-Menge aus AC2 derselben Spec, keine zweite Definition). **Frontend-Projekte** (`html`/`flutter`/`angular`): Schritt komplett überspringen — **kein** Endpunkt-Scaffold, **keine** Board-Story; dort reicht das statische `version.json`-Stempel-Artefakt aus dem Build-Versionierungs-Muster (AC6).
   1. **Spec-Vorlage kopieren** (idempotent — nur falls `docs/specs/version-endpoint.md` noch nicht existiert): `cp ${CLAUDE_PLUGIN_ROOT}/templates/_docs/specs/version-endpoint.md docs/specs/version-endpoint.md`.
   2. **Board-Story „Version-Endpunkt" idempotent anlegen** (Titel-/Spec-Match — nie doppelt):
      ...
      - Story: existiert bereits eine Story mit Titel „Version-Endpunkt" **und** `spec: docs/specs/version-endpoint.md` unter diesem Parent (`board list --type story --parent <F-###>`) → **nichts tun**. Sonst: `board story add --parent <F-###> --title "Version-Endpunkt" --spec docs/specs/version-endpoint.md --implements AC1,AC2,AC3,AC4 --priority P2` (Status `To Do` per CLI-Default).
...
- **Admin-Bereich-Scaffold (Schritt 4g) läuft nur bei UI-Projekten** (Spec `docs/specs/admin-bereich-scaffolding.md` AC4/AC6) und ist idempotent — bestehende `docs/specs/admin-bereich.md`/`config/admin-manifest.yaml`/`scripts/set-admin-password.sh` werden nie überschrieben, die Board-Story „Admin-Bereich" wird nie doppelt angelegt (Titel-/Spec-Match). Die stack-spezifische Implementierung (Routen/Session/UI-Rendering) ist explizites Nicht-Ziel dieses Schritts (AC8) — sie entsteht projekt-lokal über `/flow`.
- **Versions-Endpunkt-Scaffold (Schritt 4h) läuft nur bei Service-Projekten** (Spec `docs/specs/build-version-stamping.md` AC8) und ist idempotent — eine bestehende `docs/specs/version-endpoint.md` wird nie überschrieben, die Board-Story „Version-Endpunkt" wird nie doppelt angelegt (Titel-/Spec-Match). Die stack-spezifische Route-Implementierung ist explizites Nicht-Ziel dieses Schritts — sie entsteht projekt-lokal über `/flow`.
