...
if [[ -z "${METRICS_ROOT:-}" ]]; then
  echo "[metrics-append-dispatch] WARN: METRICS_ROOT nicht gesetzt — Ledger-Pfad unbestimmbar, Zeile nicht geschrieben" >&2
  exit 0
fi
...
if [[ ! -f "$REPO_ROOT/board/board.yaml" ]]; then
  echo "[metrics-append-dispatch] WARN: METRICS_ROOT ($REPO_ROOT) enthält kein board/board.yaml — Zeile nicht geschrieben" >&2
  exit 0
fi
