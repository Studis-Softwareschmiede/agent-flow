...
# ===========================================================================
# Test 18: Verworfen — CLI-Enum-Guard akzeptiert Status (AC1/AC2)
# @trace story-status-verworfen#AC1
# @trace story-status-verworfen#AC2
# ===========================================================================
echo ""
echo "--- Test 18: Verworfen — CLI akzeptiert status Verworfen (AC1/AC2) ---"

T18_DIR="${TEST_WORK_DIR}/test18"
setup_board "$T18_DIR"
# Blocked-Story mit gesetztem blocked_reason (make_story 7. Arg), damit AC2
# (Löschung von blocked_reason beim Übergang nach Verworfen) prüfbar ist.
make_story "$T18_DIR" "S-001" "Blocked" "docs/specs/test.md" "AC1" "" "warte auf X"

T18_EXIT=0
T18_OUTPUT=""
set +e
T18_OUTPUT="$(cd "$T18_DIR" && BOARD_DIR=board BOARD_WRITER=flow bash "$BOARD_SCRIPT" set S-001 status Verworfen 2>&1)"
T18_EXIT=$?
set -e
echo "$T18_OUTPUT"

if [[ $T18_EXIT -eq 0 ]]; then
  pass "Test 18a: set status Verworfen → Exit 0 (kein ENUM-INVALID)"
else
  fail "Test 18a: set status Verworfen → Exit ${T18_EXIT} (erwartet 0)"
fi

T18_STATUS="$(cd "$T18_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" show S-001 | python3 -c 'import sys,json; print(json.load(sys.stdin)["status"])')"
if [[ "$T18_STATUS" == "Verworfen" ]]; then
  pass "Test 18b: Status persistiert als 'Verworfen'"
else
  fail "Test 18b: Status='${T18_STATUS}' (erwartet 'Verworfen')"
fi

# AC2: done_at bleibt null, blocked_reason gelöscht
T18_DONE_AT="$(cd "$T18_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" show S-001 | python3 -c 'import sys,json; print(json.load(sys.stdin).get("done_at"))')"
if [[ "$T18_DONE_AT" == "None" || -z "$T18_DONE_AT" ]]; then
  pass "Test 18c: done_at bleibt null (Verworfen ≠ erfolgreich)"
else
  fail "Test 18c: done_at='${T18_DONE_AT}' (erwartet null)"
fi

T18_BR="$(cd "$T18_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" show S-001 | python3 -c 'import sys,json; print(json.load(sys.stdin).get("blocked_reason"))')"
if [[ "$T18_BR" == "None" || -z "$T18_BR" ]]; then
  pass "Test 18d: blocked_reason gelöscht (Status ≠ Blocked)"
else
  fail "Test 18d: blocked_reason='${T18_BR}' (erwartet null)"
fi

# ===========================================================================
# Test 19: Verworfen — Single-Writer bleibt aktiv (AC2)
# @trace story-status-verworfen#AC2
# ===========================================================================
echo ""
echo "--- Test 19: Verworfen — ohne BOARD_WRITER=flow abgewiesen (AC2) ---"

T19_DIR="${TEST_WORK_DIR}/test19"
setup_board "$T19_DIR"
make_story "$T19_DIR" "S-001" "To Do" "docs/specs/test.md" "AC1" "" ""

T19_EXIT=0
set +e
(cd "$T19_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" set S-001 status Verworfen) >/dev/null 2>&1
T19_EXIT=$?
set -e

if [[ $T19_EXIT -ne 0 ]]; then
  pass "Test 19: set status Verworfen ohne BOARD_WRITER=flow → abgewiesen (Exit ${T19_EXIT})"
else
  fail "Test 19: Single-Writer nicht erzwungen (Exit 0 trotz fehlendem BOARD_WRITER)"
fi

# ===========================================================================
# Test 20: Ungültiger Statuswert bleibt ENUM-INVALID (E1)
# @trace story-status-verworfen#AC1
# ===========================================================================
echo ""
echo "--- Test 20: Ungültiger Statuswert weiter abgewiesen (E1) ---"

T20_DIR="${TEST_WORK_DIR}/test20"
setup_board "$T20_DIR"
make_story "$T20_DIR" "S-001" "To Do" "docs/specs/test.md" "AC1" "" ""

T20_EXIT=0
set +e
(cd "$T20_DIR" && BOARD_DIR=board BOARD_WRITER=flow bash "$BOARD_SCRIPT" set S-001 status Discarded) >/dev/null 2>&1
T20_EXIT=$?
set -e

if [[ $T20_EXIT -ne 0 ]]; then
  pass "Test 20: 'Discarded' (nicht im Enum) → abgewiesen (Exit ${T20_EXIT})"
else
  fail "Test 20: ungültiger Statuswert nicht abgewiesen (Naming-Vertrag AC10)"
fi

# ===========================================================================
# Test 21: Depends-Gate terminal — Verworfen erfüllt Vorbedingung (AC3)
# @trace story-status-verworfen#AC3
# ===========================================================================
echo ""
echo "--- Test 21: board next — verworfene depends erfüllt Gate (AC3) ---"

T21_DIR="${TEST_WORK_DIR}/test21"
setup_board "$T21_DIR"
# S-001 verworfen (Vorbedingung), S-002 To Do hängt an S-001
make_story "$T21_DIR" "S-001" "Verworfen" "docs/specs/test.md" "AC1" "" ""
make_story "$T21_DIR" "S-002" "To Do" "docs/specs/test.md" "AC2" "S-001" ""

T21_OUTPUT="$(cd "$T21_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" next 2>&1 || true)"
echo "$T21_OUTPUT"
T21_NEXT_ID="$(echo "$T21_OUTPUT" | python3 -c 'import sys,json
s=sys.stdin.read().strip()
print(json.loads(s)["id"] if s else "")' 2>/dev/null || echo "")"

if [[ "$T21_NEXT_ID" == "S-002" ]]; then
  pass "Test 21a: S-002 freigegeben (verworfene Vorbedingung = terminal)"
else
  fail "Test 21a: next lieferte '${T21_NEXT_ID}' (erwartet S-002)"
fi

# ===========================================================================
# Test 21b: board ready — R4-Depends-Gate terminal (AC3, Gegenstück zu Test 21)
# @trace story-status-verworfen#AC3
# ===========================================================================
echo ""
echo "--- Test 21b: board ready — verworfene depends erfüllt R4 (AC3) ---"

T21B_DIR="${TEST_WORK_DIR}/test21b"
setup_board "$T21B_DIR"
make_active_spec "${T21B_DIR}/docs/specs/test.md" "AC1" "AC2"
make_story "$T21B_DIR" "S-001" "Verworfen" "docs/specs/test.md" "AC1" "" ""
make_story "$T21B_DIR" "S-002" "To Do" "docs/specs/test.md" "AC2" "S-001" ""

T21B_EXIT=0
T21B_OUTPUT=""
set +e
T21B_OUTPUT="$(cd "$T21B_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" ready 2>&1)"
T21B_EXIT=$?
set -e
echo "$T21B_OUTPUT"

if [[ $T21B_EXIT -eq 0 ]]; then
  pass "Test 21b-a: Exit 0 (verworfene Vorbedingung erfüllt R4)"
else
  fail "Test 21b-a: Exit ${T21B_EXIT} (erwartet 0)"
fi

if echo "$T21B_OUTPUT" | grep -q "^READY     S-002"; then
  pass "Test 21b-b: S-002 als READY gemeldet (R4 terminal wie board next, AC3)"
else
  fail "Test 21b-b: S-002 nicht READY — R4 wertet Verworfen evtl. noch nicht terminal"
  echo "  Output: $T21B_OUTPUT"
fi

# ===========================================================================
# Test 22: board next wählt NIE eine verworfene Story (AC4)
# @trace story-status-verworfen#AC4
# ===========================================================================
echo ""
echo "--- Test 22: board next — Verworfen nie Kandidat (AC4) ---"

T22_DIR="${TEST_WORK_DIR}/test22"
setup_board "$T22_DIR"
# Nur eine verworfene Story → next darf sie NICHT aufgreifen (leere Ausgabe)
make_story "$T22_DIR" "S-001" "Verworfen" "docs/specs/test.md" "AC1" "" ""

T22_OUTPUT="$(cd "$T22_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" next 2>&1 || true)"
if [[ -z "$(echo "$T22_OUTPUT" | tr -d '[:space:]')" ]]; then
  pass "Test 22: next liefert leer (Verworfen ist kein Kandidat)"
else
  fail "Test 22: next lieferte '${T22_OUTPUT}' (erwartet leer)"
fi

# ===========================================================================
# Test 23: Rollup/Progress terminal — verworfen-Suffix + done-Zähler (AC5)
# @trace story-status-verworfen#AC5
# ===========================================================================
echo ""
echo "--- Test 23: board rollup — verworfen separat, nicht in done (AC5) ---"

T23_DIR="${TEST_WORK_DIR}/test23"
setup_board "$T23_DIR"
# 1× Done, 2× Verworfen → progress "1/3 done · 2 verworfen"; Vollständigkeit 1+2==3
make_story "$T23_DIR" "S-001" "Done" "docs/specs/test.md" "AC1" "" ""
make_story "$T23_DIR" "S-002" "Verworfen" "docs/specs/test.md" "AC2" "" ""
make_story "$T23_DIR" "S-003" "Verworfen" "docs/specs/test.md" "AC3" "" ""

(cd "$T23_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" rollup F-001) >/dev/null 2>&1
T23_PROGRESS="$(cd "$T23_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" show F-001 | python3 -c 'import sys,json; print(json.load(sys.stdin)["progress"])')"
echo "progress: $T23_PROGRESS"

if [[ "$T23_PROGRESS" == "1/3 done · 2 verworfen" ]]; then
  pass "Test 23a: progress = '1/3 done · 2 verworfen' (done-Zähler unverfälscht, Suffix ausgewiesen)"
else
  fail "Test 23a: progress='${T23_PROGRESS}' (erwartet '1/3 done · 2 verworfen')"
fi

# Präfix formatstabil → ROLLUP-STALE bleibt grün
if [[ "$T23_PROGRESS" == "1/3 done"* ]]; then
  pass "Test 23b: Präfix '<done>/<total> done' formatstabil (ROLLUP-STALE-Schutz)"
else
  fail "Test 23b: Präfix nicht formatstabil ('${T23_PROGRESS}')"
fi

# ===========================================================================
# Test 24: Rollup — kein verworfen-Suffix wenn N==0 (AC5)
# @trace story-status-verworfen#AC5
# ===========================================================================
echo ""
echo "--- Test 24: board rollup — kein Suffix ohne verworfene Stories (AC5) ---"

T24_DIR="${TEST_WORK_DIR}/test24"
setup_board "$T24_DIR"
make_story "$T24_DIR" "S-001" "Done" "docs/specs/test.md" "AC1" "" ""
make_story "$T24_DIR" "S-002" "To Do" "docs/specs/test.md" "AC2" "" ""

(cd "$T24_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" rollup F-001) >/dev/null 2>&1
T24_PROGRESS="$(cd "$T24_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" show F-001 | python3 -c 'import sys,json; print(json.load(sys.stdin)["progress"])')"
echo "progress: $T24_PROGRESS"

if [[ "$T24_PROGRESS" != *"verworfen"* ]]; then
  pass "Test 24: kein 'verworfen'-Suffix bei N==0 ('${T24_PROGRESS}')"
else
  fail "Test 24: unerwartetes 'verworfen'-Suffix ('${T24_PROGRESS}')"
fi

# ===========================================================================
# Test 25: board lint akzeptiert Verworfen-Story (AC1)
# @trace story-status-verworfen#AC1
# ===========================================================================
echo ""
echo "--- Test 25: board lint — Verworfen ist im Enum (AC1) ---"

T25_DIR="${TEST_WORK_DIR}/test25"
setup_board "$T25_DIR"
make_story "$T25_DIR" "S-001" "Verworfen" "docs/specs/test.md" "AC1" "" ""

T25_OUTPUT="$(cd "$T25_DIR" && BOARD_DIR=board bash "$BOARD_SCRIPT" lint 2>&1 || true)"
if echo "$T25_OUTPUT" | grep -q "ENUM-INVALID.*status.*Verworfen"; then
  fail "Test 25: lint meldet ENUM-INVALID für Verworfen"
  echo "  Output: $T25_OUTPUT"
else
  pass "Test 25: lint akzeptiert Verworfen (kein ENUM-INVALID)"
fi
