...
# /requirement [--cost <mode>] <vage Anforderung>

cwd = Ziel-Projekt-Repo.

0. **Cost-Mode auflösen:** Präzedenz `--cost`-Argument > `profile.cost_mode` > `balanced` (Kurzformen `low`/`max`/`front` normalisieren; `front`→`frontier`). Beim Task-Dispatch in Schritt 2 den `model`-Parameter aus `${CLAUDE_PLUGIN_ROOT}/knowledge/model-tiers.md` (Rolle `requirement`) mitgeben; bei `balanced` **keinen** Override (Frontmatter `opus` gilt). Das `--cost`-Token gehört NICHT zur Anforderung — vor dem Agent-Dispatch entfernen.
