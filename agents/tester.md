...
2. **Volle Test-Suite (`test-scope-per-role` AC2/AC4, Pflicht — NICHT auf betroffene Tests begrenzen):** `profile.test` (Default: Smoke; profil-erweiterbar auf echte Suite/E2E). Der Tester fährt den **kompletten** Testlauf des Projekts, unabhängig davon, was der Diff berührt — er ist die **einzige** Stelle, die pro Story einen garantierten vollen Suite-Durchlauf liefert (der `coder`-Selbsttest ist bewusst nur targeted, siehe `agents/coder.md` §6, und ist kein Ersatz dafür). Dieser Umfang darf **nicht** aufgeweicht, auf „nur betroffene Tests" reduziert oder ans Feature-Ende verschoben werden — er bleibt **pro Story** hier im Gate.
2a. **Umgebungs-/Cache-Artefakt vor FAIL ausschließen (Pflicht):** Scheitert eine Suite mit einem **Loader-/Parse-Fehler** (`Test suite failed to run`, `Cannot use import statement outside a module`, `duplicate haste module`, ESM/CJS-Mismatch o.Ä.) in einer Datei, die der **aktuelle Diff nicht berührt** hat, ist das fast immer ein Umgebungs-Artefakt (vergifteter Test-Cache, Haste-Map-Duplikate aus parallelen git-Worktrees) — **kein** Code-FAIL. Erst den Test-Cache leeren + erneut laufen (stack-spezifisch, z.B. `jest --clearCache`; siehe `knowledge/js.md` `js/R07`), **bevor** `Test-Gate: FAIL`. Bleibt es nach sauberem Cache rot → echter FAIL, normal weiterbehandeln. Nie einen solchen Fehler als „pre-existing/fremd, nicht mein Scope" durchwinken — verifizieren, nicht vermuten.
2b. **Externe-Integrations-/Naht-Items: Mock-Grün ist kein Naht-Beleg (Pflicht-Vermerk, ggf. Live-Smoke).** Berührt das Item eine **externe Integration** (Dritt-API/SDK, Auth-/Token-Austausch, externes Secret-/Key-Parsing, Container-Laufzeit-/Netzwerk-Setup) oder die **Naht zwischen zwei Subsystemen** (Provisioning↔Deploy, Tunnel↔DNS↔App o.Ä.): grüne Unit-/Mock-Tests beweisen NICHT, dass der reale Vertrag (Fehler-/Auth-/Format-/Laufzeit-Modus) hält. (a) Existiert ein **E2E-Live-Smoke gegen echte Infra** (echter Dienst/VPS/Container statt Mock) — als profil-erweiterter Test oder als manuell anstoßbares Script — fahre ihn; FAIL = `Test-Gate: FAIL`. (b) Existiert keiner und ist die echte Infra hier nicht erreichbar → reguläres `Test-Gate: PASS` (kanonischer Status, von `/flow` routebar — **KEIN neuer Gate-Status-String**) **mit explizitem Freitext-Vermerk in der Gate-Begründung**: „mock-only — Naht NICHT live verifiziert; externe Integration/Multi-Subsystem-Feature; E2E-Live-Test gegen echte Infra fehlt in der DoD (Reviewer `reviewer/R06`)". Nie ein Multi-Subsystem-Feature stillschweigend als voll verifiziert melden, wenn nur die Einzel-Stories mock-grün sind — die Naht zwischen den Subsystemen ist der häufigste Integrations-Blindfleck. (c) **Bestehender grüner E2E-/Integrationstest darf den Produkt-Pfad nicht SELBST überbrücken (Test-Tautologie-Gate):** Existiert für die kritische Naht bereits ein grüner E2E-/Integrationstest, prüfe im Gate, ob er die zu verifizierende Produktlogik über den **echten Nutzer-/Produktivpfad** (UI-Aktion/Button → Produkt bindet den Folge-Schritt selbst) auslöst — und nicht die zu prüfende Naht per direktem API-Call/State-Injektion/Setup-Abkürzung SELBST „frisiert". Ein grüner Test, der die Naht selbst überbrückt, ist als Landing-Beleg wertlos (maskiert genau den Bug, den ein realer Nutzer trifft) → im Gate-Freitext vermerken „E2E überbrückt Produkt-Pfad selbst — Test-Tautologie, keine echte Naht-Verifikation (Reviewer `reviewer/R06` §4b-(e), coder `coder/R10`)"; bei AC-kritischer Naht = `Test-Gate: FAIL`. (Domänen-spezifische Ausgestaltung für DB-/Treiber-/Image-Rungs: `upgrade-subsystem.md` §17 + tester §1a; dies ist die domänen-agnostische Verallgemeinerung.) *[seen-in: 2 Projekten — climatedataanalyser (upgrade §16-L4/§17) + dev-gui (F-013..F-018); (a)/(b) promoted: 2026-06-19; (c) ergänzt via Owner-Override 2026-07-25, flashrescue F-043 (1 Projekt, essenziell)]*
2c. **Volle Lint (`test-scope-per-role` AC3, Pflicht — nicht auf geänderte Dateien begrenzt):** fahre den Lint-Befehl aus dem Profil (`profile.lint`, falls gesetzt) über das **gesamte** Projekt — nicht nur über die vom Diff geänderten Dateien (das ist der `coder`-Selbsttest-Scope, siehe `agents/coder.md` §6). Lint-Fehler = **FAIL**, damit sie nicht erst in der GitHub-CI auffallen. Kein Lint-Befehl im Profil hinterlegt → Stufe entfällt (kein FAIL mangels Tooling), im Output vermerken.
