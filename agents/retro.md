...
4. Branch anlegen; Änderung in `${CLAUDE_PLUGIN_ROOT}/knowledge/<x>.md` (bzw. Agent-Def), jede neue Regel mit **stabiler ID** (`<pack>/R<NN>`).
5. PR öffnen + Zeile in `${CLAUDE_PLUGIN_ROOT}/LEARNINGS.md` (Status `Proposed`) + Improvement-Board-Karte.

# Output
PR-Link + Liste: `promote → ${CLAUDE_PLUGIN_ROOT}/knowledge/<x>: <Regel> [ID]`.
