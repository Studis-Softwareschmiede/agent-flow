...
4. Promotion vorbereiten: Änderung in `knowledge/<lang>.md`, **jede Regel mit autoritativer Quelle (Link) + stabiler ID** (`<lang>/R<NN>`). Bei `/train security`: zusätzlich das **`last_trained:`**-Datum oben in `knowledge/security.md` auf **heute** setzen (Frische-Signal; auch wenn keine neue Regel rauskommt — dann nur das Datum). **Quellen-Bindung (verbindlich, genau wie bei Framework-Packs):** der `security`-Domänen-Pack führt seinen `primary_sources`/`non_sources`-Block **nicht** als YAML-Frontmatter, sondern als Prosa-Sektion **`## Quellen (train-verbindlich)`** im Pack-Kopf (Blockquote mit `primary_sources:`/`non_sources:`). `/train security` recherchiert **ausschließlich** aus diesen `primary_sources` (OWASP Top 10, NIST CSRC/SP 800-63/SP 800-131, IETF Datatracker/RFC, PortSwigger Web Security Academy) und **ignoriert** jeden Treffer aus `non_sources` (dev.to, medium.com, stackoverflow.com, geeksforgeeks.org) — nie daraus zitieren. Der Block wird aus der Sektion `## Quellen` gelesen, **nicht** aus einem YAML-Header (den hat `security.md` nicht).
