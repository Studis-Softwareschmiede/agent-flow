...
1a. **Admin-Manifest-Intake** (nur bei **UI-Projekten**, **einmalig** bei App-Erstellung — `docs/specs/admin-bereich-manifest-intake.md` AC1–AC3, `docs/architecture/admin-bereich-subsystem.md` §7/BR-011): eigener, einmaliger Fragen-Schritt **ausserhalb** des Rückfrage-Loop-Budgets aus Schritt 2 (analog zum separaten designer-Dispatch in Schritt 4) — zählt nicht gegen dessen 2–3-Fragen-Limit.
   - **UI-Projekt-Erkennung** (identisch zu `docs/specs/design-owner-approval.md` „UI-Projekt-Erkennung"): `profile.language` ∈ `flutter|angular|html` (bei Array-Form genügt ein Treffer) **oder** `profile.domains` enthält `ui`/`accessibility`. Trifft das nicht zu → Schritt vollständig überspringen (AC1, GE7).
   - **Einmaligkeits-Signal:** `config/admin-manifest.yaml` fehlt **oder** trägt noch `parameters: []` (frisch gescaffoldeter Zustand, `templates/_shared/admin/admin-manifest.yaml`) → die Frage ist fällig. Trägt sie bereits Einträge → schon befüllt, Schritt überspringen (kein wiederholtes Fragen).
   - **Startbestand:** vorhandene `.env`/`.env.example`-Keys im Ziel-Repo lesen (Key-**Namen**, keine Werte). Keine Keys vorhanden → Frage entfällt, es bleibt beim leeren Manifest-Gerüst (Edge-Case der Spec).
   - **Frage stellen (AskUserQuestion, Mehrfachauswahl):** welche der gelesenen `.env`-Keys ins Admin-Manifest sollen — Startbestand = alle gelesenen Keys als Optionen.
   - **Antwort verarbeitet (AC2):** für jeden ausgewählten Key einen Eintrag in `config/admin-manifest.yaml` gemäss Manifest-Vertrag (→ BR-011, `docs/architecture/admin-bereich-subsystem.md` §4): `key`, `quelle: env`, `typ` (`secret` bei Secret-Namensmuster `*_KEY`/`*_TOKEN`/`*_SECRET`/`*_PASSWORD*`/`*_URL`, sonst `string`), `editierbar: false`, `secret` (`true` bei Secret-Namensmuster, sonst `false`), `validierung` (leer — Verfeinerung projekt-lokal, GE6). Dieselben Feld-Defaults gelten unabhängig davon, ob der Key vom Owner ausgewählt wurde oder (A1) autonom übernommen wird — die Owner-Antwort steuert nur, **welche** Keys aufgenommen werden.
   - **A1 — autonomer Lauf ohne Antwort (AC3):** kein Owner erreichbar → konservativer Default statt Blockade: **alle** gelesenen `.env`-Keys werden mit denselben Feld-Defaults aufgenommen (keine Auswahl-Einschränkung).
   - Schreibt nur in den Working-Tree (kein Commit) — das Committen macht wie gehabt der `/requirement`-Skill nach dem Lauf.
...
3. **Bereichs-Gate** (`docs/specs/requirement-area-intake.md` AC1–AC3, AC5, AC6 — vor jeder Spec-/Story-Anlage):
   - **Bereichsliste laden (AC1):** `board area list` ausführen (JSON `[{id,name,description,order}]`). Leeres Array (`areas.yaml` fehlt/leer) → **Bereichs-Gate deaktiviert**: mit Schritt 4 wie bisher (ohne Bereichs-Zuordnung) fortfahren; im Lauf-Output vermerken: „kein Bereichs-Gate: areas.yaml fehlt".
   ...
   - **Zuordnung zu genau einem bestehenden Bereich (AC2):** anhand `name`/`description` der Bereichsliste denjenigen bestehenden Bereich bestimmen, zu dem die (Teil-)Anforderung inhaltlich gehört. Diese Zuordnung bestimmt später: Story-`parent` = das Bereichs-Feature dieses Bereichs (Feature mit `area: <bereich-id>`); neue Specs werden mit `area: <bereich-id>` gestempelt (Schritt 4).
...
6. Pro TODO ein GitHub-Issue + aufs Board (Status **To Do**), Body:
   ...
   - **UI-Label (`docs/specs/design-owner-approval.md` AC4):** Hat die Story einen sichtbaren UI-Anteil (dieselbe Einschätzung wie beim Designer-Dispatch in Schritt 4) → Label `ui` am Board-Item vergeben (`--labels ui` bei `board story add` bzw. `gh issue edit --add-label ui` im GitHub-Pfad, je nach `profile.board`). Das Label ist die **deterministische** Grundlage für das `/flow`-Bau-Gate (AC6) — keine inhaltliche Diff-/Spec-Analyse dort nötig. Stories ohne sichtbaren UI-Anteil erhalten das Label nicht (Bestandsschutz, AC8).
