...
3. **Bereichs-Gate** (`docs/specs/requirement-area-intake.md` AC1–AC3, AC5, AC6 — vor jeder Spec-/Story-Anlage):
   ...
   - **Kein passender Bereich, aber kein Konzept-Widerspruch (bereichsfremd / Grenzfall unklar — AC4/AC7)** → **keine** Story/Spec für diesen Teil anlegen. Stattdessen schreibe einen Eintrag in die **Ideen-Inbox** (`docs/ideas-inbox.md`). Ein unklarer Grenzfall (mehrdeutig, aber kein Widerspruch) nimmt **dieselbe** Route (konservativ, keine Owner-Rückfrage) — die Begründung nennt die Unschärfe.

     **Ideen-Inbox-Eintrag (AC4/AC7):** Schreibe einen Markdown-Eintrag **append-only** (bestehende Einträge nie überschreiben):
     ```markdown
     ### <kurzer Titel der Anforderung>

     - status: Idee
     - created_at: <ISO-8601-UTC-Zeitstempel, z.B. 2026-07-04T12:30:00Z>
     - begruendung: <genau 1 Satz: warum keinem bestehenden Bereich zugeordnet>

     <Anforderungs-Beschreibung: 1–3 Sätze, was gefordert ist>
     ```
     **Mechanik:** Append an die Datei `docs/ideas-inbox.md` (nach dem Header-Block, **newest-first** — neue Einträge oben). `created_at` ist die aktuelle UTC-Zeit beim Schreiben. Begründung ist prägnant und nachvollziehbar (z.B. „passt zu keinem der Bereiche flow-orchestrierung, board, spezifikation — könnte später ein eigenständiger Bereich werden" oder „ist Grenzfall zwischen board + flow-orchestrierung, daher konservativ für später").
